...
import { authApi, roleRequirementsApi, skillsApi, PathwaySkill, Skill, categoriesApi } from '../services/api';
...
export const CareerPathways: React.FC = () => {
  ...
  const handleAddSkill = async () => {
    ...
    setSaving(true);
    try {
      await roleRequirementsApi.addSkillToPathway(selectedSkillToAdd);
      setMessage({ type: 'success', text: 'Skill added to pathways with default requirements' });
  ...
  const handleAddAllSkillsForPathway = async () => {
      ...
      <header className="bg-[#F6F2F4] shadow-sm border-b border-gray-200 -mx-4">
        <div className="w-full px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-gray-800 italic" style={{ fontFamily: '"Times New Roman", Times, serif', fontStyle: 'italic' }}>Career Pathways</h1>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              >
                ...
                {pathwaysList.map(p => (
                  <option key={p.pathway} value={p.pathway}>
                    {p.pathway} ({p.total_skills} skills)
                  </option>
                ))}
              </select>
              {currentPathwayInfo && (
            ...
            {selectedPathway && currentPathwayInfo && currentPathwayInfo.skills_remaining > 0 && (
              <button
                onClick={handleAddAllSkillsForPathway}
              ...
              >
                {addingAllSkills ? (
                  ...
                  <>Import Skills ({currentPathwayInfo.skills_remaining} remaining)</>
                )}
              </button>
            )}
          </div>
        ...
        {selectedPathway && (
          <div className="bg-white rounded-lg shadow-sm overflow-hidden">
            ...
            <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
              ...
              >
                <span>+</span> Add Skill Requirement
              </button>
            </div>
            ...
            {skillCategories.length > 0 ? (
                                ...
                                <td className="px-4 py-2 text-center">
                                  {editingSkill === skill.skill_id ? (
                                    <div className="flex justify-center gap-1">
                                      <button
                                        onClick={() => handleSaveSkill(skill.skill_id)}
                                        disabled={saving}
              ...
              <div className="p-8 text-center text-gray-500">
                No skills configured for this pathway yet. Click "Import Skills" to add all skills from this pathway, or "Add Skill Requirement" to add individual skills.
              </div>
            )}
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
              ...
              <select
                ...
                onChange={(e) => setSelectedSkillToAdd(Number(e.target.value) || null)}
