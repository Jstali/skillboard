...
class HRMSProjectSync(BaseModel):
    hrms_project_id: str
    name: str
...
@router.post("/sync-from-hrms", response_model=ProjectSchema)
async def sync_project_from_hrms(
    project_data: HRMSProjectSync,
    db: Session = Depends(database.get_db),
    current_user: User = Depends(get_current_user)
):
    """
    Sync a project from HRMS to local database.
    Creates the project if it doesn't exist, updates if it does.
    """
    # Check if project already exists by HRMS ID
    existing = db.query(Project).filter(
        Project.hrms_project_id == project_data.hrms_project_id
    ).first()
    
    if existing:
        # Update existing project
        existing.name = project_data.name
        existing.client_name = project_data.client_name
        existing.status = project_data.status
        existing.manager_name = project_data.manager_name
        db.commit()
        db.refresh(existing)
        return existing
    ...
    new_project = Project(
        ...
        hrms_project_id=project_data.hrms_project_id,
        client_name=project_data.client_name,
        status=project_data.status,
        manager_name=project_data.manager_name
    )
    db.add(new_project)
    db.commit()
...
@router.get("/by-hrms-id/{hrms_project_id}", response_model=ProjectSchema)
async def get_project_by_hrms_id(
    hrms_project_id: str,
    db: Session = Depends(database.get_db),
    current_user: User = Depends(get_current_user)
):
    """
    Get a project by its HRMS project ID.
    """
    project = db.query(Project).filter(
        Project.hrms_project_id == hrms_project_id
    ).first()
